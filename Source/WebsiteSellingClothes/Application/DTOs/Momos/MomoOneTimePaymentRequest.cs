...
public class MomoOneTimePaymentRequest
{
    ...
    public MomoOneTimePaymentRequest(string partnerCode, string requestId, long amount, string orderId, string orderInfo, string redirectUrl, string ipnUrl, string requestType, string extraData, string lang)
    {
    ...
    public void MakeSignature(string accessKey, string secretKey)
    {
        ...
        this.signature = HashHelper.HmacSHA256(secretKey,rawSignature);   
    }
    public async Task<(bool,string?)> GetLink(string paymentUrl)
    {
        ...
        var createPaymentLinkResponse = await client.PostAsync(paymentUrl, requestContent);
        ...
        if (createPaymentLinkResponse.IsSuccessStatusCode)
        {
            ...
            var responseData = JsonConvert.DeserializeObject<MomoOneTimePaymentLinkResponse>(responseContents);
